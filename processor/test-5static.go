...
var (
	...
	runtime    = time.Minute * 10 // 测试时间
	// scaleInterval  = time.Second * 10 // 弹性伸缩间隔
...
func run(ctx context.Context, scaleRequest *pb.ScaleRequest, client pb.OpenGaussControllerClient) {
	...
	scaleRequest.WorkerReplication = 5
	...
	for {
		...
		default:
			...
			// 均为string
			scaleRequest.WorkerReplication = 5
			if scaleRequest.WorkerReplication != 5 {
				lastScaleTime = time.Now()
				scaleRequest.WorkerReplication = 5
				response, err := client.Scale(context.TODO(), scaleRequest)
				if err != nil {
