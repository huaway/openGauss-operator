...
func run(ctx context.Context, scaleRequest *pb.ScaleRequest, client pb.OpenGaussControllerClient) {
	...
	for {
		...
		case <-ctx.Done():
			return
		default:
			// 查询第一组主机的CPU利用率，以决定是否增加主机
			result, err := prometheusUtil.QueryMasterCpuUsagePercentage("a", queryClient)
			if err != nil {
			...
			for _, v := range m {
				...
				fmt.Println("Cluster a's Master CPU Usage", percentage)
				// ----------start-----------
				// 弹性伸缩相关代码：
				// 发起rpc调用
				if isScale {
					...
					if percentage > 1000 {
						if time.Since(lastScaleTime) < scaleInterval || isMulti {
							continue
						}
						lastScaleTime = time.Now()
						isMulti = true
						cmd := exec.Command("bash", `./add-master.sh`)
					...
					if percentage < 200 {
						...
						lastScaleTime = time.Now()
						isMulti = false
						cmd := exec.Command("bash", `./remove-master.sh`)
						response, err := cmd.Output()
						if err != nil {
							log.Fatal(err)
						}
